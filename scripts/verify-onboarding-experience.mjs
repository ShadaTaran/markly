...
check("P9: Stage 35 itself created no migration — 0016 was still the latest migration at the time of this stage's own work (Stage 37 added 0017, the Stage 40 data-integrity correction added 0018, and Stage 42's own unrelated source-delete policy correction added 0019, all legitimately; this snapshot is bumped forward each time a later stage adds one, same convention as every other stage's own version of this check)", () => {
  ...
  assert.ok(highest.startsWith("0019_"), `expected 0019_stage42_source_delete_policy.sql to be the latest migration, found ${highest}`);
});
