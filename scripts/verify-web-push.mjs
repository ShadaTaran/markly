...
check("G14 (test §66): migrations 0001-0016 remain untouched — 0017 was additive when this stage shipped, the Stage 40 data-integrity correction's own 0018 came later, and Stage 42's own unrelated source-delete policy correction's 0019 is now the latest (bumped forward, same convention as every other stage's version of this check)", () => {
  const files = readdirSync("supabase/migrations").filter((name) => name.endsWith(".sql")).sort();
  const highest = files[files.length - 1];
  assert.equal(highest, "0019_stage42_source_delete_policy.sql");
